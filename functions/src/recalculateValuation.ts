...
export const recalculateValuation = onRequest(
...
User-Provided Free Text Notes / Characteristics: "${freeTextNotes || 'None'}"`;
      ...
      console.error("Error in recalculateValuation:", err);
      res.status(500).json({ error: err.message || "Failed to recalculate valuation." });
    }
  }
);

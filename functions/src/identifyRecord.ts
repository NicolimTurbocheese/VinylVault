...
import { geminiApiKeySecret } from "./_lib/secrets";

export const identifyRecord = onRequest(
  { secrets: [geminiApiKeySecret], cors: false, timeoutSeconds: 60 },
  async (req, res) => {
...
12. If the record's country of origin is Japan (including when the supplied country field says "Japan" or the label/catalogue points to a Japanese pressing plant, e.g. Toshiba-EMI, King Records, JVC/Victor, Odeon Japan, Polydor K.K.), prioritize Japan-pressing-specific references across ALL research steps above: prefer sold listings and Discogs release entries explicitly for the Japanese pressing (not a US/UK/EU pressing of the same album), search with "Japan press" / "Japan OBI" / "国内盤" qualifiers alongside the catalogue/matrix number, and value it against completed sales of that same Japanese pressing rather than a generic international pressing — Japanese pressings routinely carry a distinct (often higher) collector value tied to OBI strip presence and pressing-plant provenance, so conflating it with a non-Japan pressing's sale data will misprice it.
...
- Estimated Base Mint Value: Low S$${discogsData.baseMintValue.low}, Median S$${discogsData.baseMintValue.median}, High S$${discogsData.baseMintValue.high} (SGD)`;
      ...
      console.error("Error in identifyRecord:", err);
      res.status(500).json({
